...
- **Изображения** — фото без E2E (хранятся/отдаются как есть через Object Storage); тексты по-прежнему E2E.
...
Фото загружаются и скачиваются **напрямую браузер ↔ Object Storage** через временные pre-signed URL, минуя nginx и Go-бэкенд. Полное описание (эндпоинты, CORS/CDN/IAM, модель приватности, очистка) — в [`docs/photo-uploads.md`](docs/photo-uploads.md).
...
- После первого входа админа клиент загружает на сервер **зашифрованную резервную копию ключей** (секрет — сам `BOOTSTRAP_TOKEN`). Повторный вход по bootstrap-ссылке **восстанавливает те же ключи** и историю сообщений (в том числе на новом устройстве / после очистки данных сайта).
- Если админ уже есть и включён `BOOTSTRAP_ALLOW_REBIND=1`, а копии нет (или вы явно подтверждаете сброс) — bootstrap может создать **новые** ключи. Тогда старые текстовые сообщения не расшифруются (фото обычно остаются). Это крайний случай, не обычный вход.
- На устройстве, где аккаунт админа уже сохранён, bootstrap входит в него **без смены ключей** и подтягивает групповые ключи из резервной копии.
...
После входа приложение запросит разрешение на уведомления и зарегистрирует устройство. Push отправляется всегда; если приложение открыто на экране, системное уведомление скрывается service worker'ом (сообщение приходит по WebSocket). В уведомлении — имя отправителя и **обрезанный текст** сообщения (до ~120 символов); для фото/видео — «Фото» / «Видео». Превью уходит на сервер только для push и **не сохраняется** (тело сообщения по-прежнему E2E). На iPhone PWA текст кладётся ещё и в Declarative Web Push (`web_push: 8030`), чтобы баннер показал его даже если service worker не успел отработать.
...
Фото **не проходят через nginx**: при включённом Object Storage браузер грузит и скачивает их напрямую в бакет через pre-signed URL (см. [`docs/photo-uploads.md`](docs/photo-uploads.md)). Жёсткий лимит размера на бэкенде по умолчанию отключён (`PHOTO_MAX_FILE_SIZE=0`); клиент сжимает фото перед загрузкой. Через nginx/Go идут только метаданные. Мультипарт-загрузка через API (`/api/chats/{id}/images`) остаётся как fallback и ограничена лимитом nginx.
