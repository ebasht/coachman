...
Фото **не проходят через nginx**: при включённом Object Storage браузер грузит и скачивает их напрямую в бакет через pre-signed URL (см. [`docs/photo-uploads.md`](docs/photo-uploads.md)). Лимит размера задаёт бэкенд (`PHOTO_MAX_FILE_SIZE`, по умолчанию 30 МБ), клиент сжимает фото перед загрузкой. Через nginx/Go идут только метаданные. Мультипарт-загрузка через API (`/api/chats/{id}/images`) остаётся как fallback и ограничена лимитом nginx.
