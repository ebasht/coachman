...
- Если админ уже есть и включён `BOOTSTRAP_ALLOW_REBIND=1` — bootstrap-ссылка может заново привязать ключи текущего админа к новому устройству. **Это создаёт новые ключи: старые текстовые сообщения на этом аккаунте не расшифруются** (фото обычно остаются). Используйте только на новом устройстве, когда локальных ключей больше нет.
- На устройстве, где аккаунт админа уже сохранён, bootstrap просто входит в него **без смены ключей** (после обычного выхода из аккаунта открывайте список аккаунтов или ту же bootstrap-ссылку — она не должна предлагать «привязку» с новыми ключами).
