...
func (c *fcmClient) send(ctx context.Context, token string, data map[string]string, title, body string, ttlSeconds int, callTag string) error {
	...
	// Call wakes and badge-only updates must be data-only so Android delivers to
	// MessagingService while backgrounded/killed (notification+data is swallowed
	// by the system tray and never reaches the app for silent badge bumps).
	dataOnly := data["type"] == "incoming-call" || data["type"] == "call-ended" || data["type"] == "badge"
	if !dataOnly && (title != "" || body != "") {
